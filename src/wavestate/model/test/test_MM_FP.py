...
def test_FP_MM(plot, tpath):
    ...
    overlapper = pa.mm.overlap(target_fr="src", target_to="cavity", Wk=1064)
...
def test_FP_MM_obj(plot, tpath):
    ...
    obj_sys["SRC1_beam/q"] = optics.ComplexBeamParam.from_W_R(
        1.500e-3, 100, wavelength_m=1064e-9
    )
    ...
    overlapper = pa.mm.overlap(target_fr="SRC1_beam", target_to="cavity", Wk=1064)
    print(overlapper.overlap)
    if plot:
        overlapper.plot(path.join(tpath, "MM_FP_plot.pdf"))
    return
...
def test_FP_MM_optimize(tpath, ic):
    ...
    ic(pa.mm._targets)
    pa.ga.graph_MM_link_matrix(
        path.join(tpath, "MM_links.pdf"), color_cavities={"cavity": "red"}
    )
