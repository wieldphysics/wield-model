...
class MirrorBase(base.OpticalObject):
            ...
            if lport_build == 'Dl!i':
                ...
                for Wk, wnval in manip.basis_wavenumbers(with_keys = True):
                    ...
                    Dl_prod_left = base.KeyMatrixSame(
                        stR = (basis['wavenumber'], base.kg_twophoton_pmAC, ),
                        stC = (basis['wavenumber'], base.kg_twophoton_pm, ),
                        kmatrix = {
                            (Wk, '+AC',) : {(Wk, '+') : [[+1j * wnval / 2]]},
                            (Wk, '-AC',) : {(Wk, '-') : [[-1j * wnval / 2]]},
                        },
                    ...
                    Dl_prod_right = base.KeyMatrixDiff(
                        stR = (),
                        stC = (basis_fr['frequency'], base.kg_twophoton_pmAC, ),
                        kmatrix = {
                            () : {(base.FrequencyKey({}), '+AC', ) : [[1]]},
                        },
                        build = True,
                    )
                ...
                Fl_prod_left = base.KeyMatrixDiff(
                    stC = (),
                    stR = (basis_to['frequency'], base.kg_twophoton_pmAC, ),
                    kmatrix = {
                        (base.FrequencyKey({}), '+AC', ) : {() : [[1]]},
                    },
                    build = True,
    ...
    def visit_mode_matching_transport(self, manip):
        ...
        if lport_fr_to in [('A!i', 'B!o'), ('A1!i', 'B1!o'), ('A2!i', 'B2!o')]:
            #AB transmission
            matrixAI_X = alm.interface_ROC_AOI_X(ROC_A_m, n_from = n_A, n_to = n_I, AOI_rad = AOI_rad)
            matrixAI_Y = alm.interface_ROC_AOI_Y(ROC_A_m, n_from = n_A, n_to = n_I, AOI_rad = AOI_rad)
            matrixII   = matrix_stack([[1, depth_m], [0, 1]])
            matrixIB_X = alm.interface_ROC_AOI_X(ROC_B_m, n_from = n_I, n_to = n_B, AOI_rad = AOI_rad, neg = True)
            matrixIB_Y = alm.interface_ROC_AOI_Y(ROC_B_m, n_from = n_I, n_to = n_B, AOI_rad = AOI_rad, neg = True)
            ...
            #BA transmission
            matrixBI_X = alm.interface_ROC_AOI_X(ROC_B_m, n_from = n_B, n_to = n_I, AOI_rad = AOI_rad)
            matrixBI_Y = alm.interface_ROC_AOI_Y(ROC_B_m, n_from = n_B, n_to = n_I, AOI_rad = AOI_rad)
            matrixII = matrix_stack([[1, depth_m], [0, 1]])
            matrixIA_X = alm.interface_ROC_AOI_X(ROC_A_m, n_from = n_I, n_to = n_A, AOI_rad = AOI_rad, neg = True)
            matrixIA_Y = alm.interface_ROC_AOI_Y(ROC_A_m, n_from = n_I, n_to = n_A, AOI_rad = AOI_rad, neg = True)
            ...
            def p_builderZ(p):
                ...
                depth_m = depth_m / np.cos(AOI_rad * n_A / n_I)
                return 2*depth_m
            manip.set_Zpropagator({'depth[m]' : (2 / np.cos(AOI_rad * n_A / n_I), 'AOI[deg]')})
        return
...
class DichroicMirror(MirrorBase):
    def __init__(self):
        ...
        self.values_settable([
            'R_default', 'R_default', 'R_default',
            'R_dichroic', 'R_dichroic', 'R_dichroic',
        ])
