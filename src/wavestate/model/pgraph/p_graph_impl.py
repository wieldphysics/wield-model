            ...
            for dep_Pid in Pid_to_Pid_deps.get(Pid, ()):
                ...
                if not robject._allow_multiple_parents:
                    ...
                    raise RuntimeError(
                        ...
                        " parents (reference aliasing)".format(self.object_path_built[robject])
                    )
                    ...
                    if Pid_test is not Pid_new:
                        ...
                        if Pid_req_test is not None:
                            ...
                            if Pid_deps_new is not None:
                                Pid_to_Pid_deps.setdefault(Pid_req_test, set()).update(Pid_deps_new)
                        else:
                            ...
                            if Pid_deps_new is not None:
                                if Pid_deps_test is None:
                                    ...
                                    Pid_to_Pid_pdeps[Pid_test] = Pid_to_Pid_pdeps[Pid_new]
                                else:
                                    Pid_deps_test.update(Pid_deps_new)
                                    Pid_to_Pid_pdeps[Pid_test].update(Pid_to_Pid_pdeps[Pid_new])
        ...
        for obj, refdict in references_intermediate.items():
            for ref, rtupdict in refdict.items():
                for rtup, Pid in rtupdict.items():
                    ...
                    do_assert = True
        if do_assert:
            raise RuntimeError("Remaining Items left in the references dictionary (check for cycles/bugs/missing constructions?)")
    ...
    def _resolve_parameter_tree(self, objects):
        ...
        def transfer_cdict(
            ...
            if ofunc is not None:
                ofunc_d_test = dict()
                ofunc_d = object_functions.setdefault((context_obj, ofunc), ofunc_d_test)
                ...
                ofunc_d['active_assignments'][(context_rtup, context_vtup)] = context_rseq
        ...
        t_seq_cdict = collections.namedtuple('seq_cdict', ['rseq', 'cdict', 'origin_obj_rtup_vtup'])
        for obj in objects:
            #print("B: ", obj)
            for (rtup, vtup), cdict in obj._value_dict.items():
                ...
                object_values_sources[obj_rseq[-1]][vtup].append(t_seq_cdict(obj_rseq, cdict, (obj, rtup, vtup)))
