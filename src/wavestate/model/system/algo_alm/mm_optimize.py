...
class ModeMatchingOverlapperOptimizing(mm_overlapper.ModeMatchingOverlapper):
    ...
    def optimize(
        ...
        def pbg_set_x(x):
            ...
            if mats is not None:
                ...
                for len_m, op, val in zip(mats.lengths_inv, mats.ops_constrain, dep_vec):
    ...
    def compile_overlap_calculation(self, pbg = None):
        ...
        def calculate_overlap(
            ...
            overlap = abs(qB.t1qX.overlap_HG(qB.t2qX) * qB.t1qY.overlap_HG(qB.t2qY))**2
            if qB.bad:
                return None
            return overlap
...
def build_length_constraints(
    pbg, bg, trans_center, length_constraints,
):
    ##########
    #  Length constraints
    Zprop_deps = []
        ...
        for wp in lc[1:]:
            ...
            for ol in olset:
                ...
                if idx is not None:
                    idxs.add(idx)
            if len(idxs) == 0:
                raise RuntimeError("Length constraint not within matching path for waypoint '{}'".format(wp))
            if len(idxs) > 1:
                raise RuntimeError("Length constraint not unique for waypoint '{}' (be more specific)".format(wp))
            idx = next(iter(idxs))
            path_idx.append(idx)
        Zprop_scales = dict()
...
def compile_length_constraints(
    ...
    for cmap in ZpropB.constraints:
        ...
        op_len_constr.append(
            cmap_k & opt_params_op
        )
