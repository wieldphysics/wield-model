...
class ModeMatchingAlgorithm(algo_mm_linkages.ModeMatchingLinkageAlgorithm):
    ...
    def target_add(
        ...
        if len(oLp_set_seq) == 0:
            raise RuntimeError((
                "Must specify at least one waypoint port"
                " to bind the target"
            ))
        ...
        if not isinstance(qX, alm.ComplexBeamParam):
            qX = alm.ComplexBeamParam(
                qX, wavelength_m = self.fs.wavelength_from_Wk(Wk)
            )
        if not isinstance(qY, alm.ComplexBeamParam):
            qY = alm.ComplexBeamParam(
                qY, wavelength_m = self.fs.wavelength_from_Wk(Wk)
            )
    ...
    def _cavity_params(self, target_name, ol, Wk):
        ...
        if not np.isfinite(abs(qX)) or not np.isfinite(abs(qY)):
            ...
            for ol1, ol2 in algo_mm_linkages.path_pairs(trans.Y.oLp_path):
                #print(ol1, ol2)
                idx1, idx2 = trans.Y.inc_ol2idx.get(ol1, None), trans.Y.inc_ol2idx.get(ol2, None)
    ...
    def cavity_parameters(
        ...
        return declarative.Bunch(
            ...
            gouyY_deg = np.angle(params.qY.propagate_matrix(params.matY).gouy_phasor / params.qY.gouy_phasor, deg = True),
        )
    ...
    def cavity_digest(self, Wk, waypoint = None):
        for target, target_op in sorted(self.cavity_map.items()):
            ...
            def print_wp(cB, wp):
                print('-------------')
                print('-----', target, '  at  ', wp)
                print('         Gouy X,Y [deg]  {:.2f}, {:.2f}'.format(cB.gouyX_deg, cB.gouyY_deg))
                print('         Gouy Frac {:.4f}, {:.4f}'.format((cB.gouyX_deg / 360 + .5) % 1 - 0.5, (cB.gouyY_deg / 360 + .5) % 1 - 0.5))
                if waypoint:
                    print('         qX ', cB.qX)
                    print('         qY ', cB.qY)
                    print('         diameter[m] X, Y ', alm.str_m(cB.qX.W * 2), alm.str_m(2 * cB.qY.W))
            if waypoint is None:
                wp = cavity_seq[0]
    ...
    def overlap(
        ...
        if not oLp_path_center:
            if (target_fr is None and target_to is None):
                raise RuntimeError("Must specify from and to targets if no waypoint path is provided")
            oLp_set_seq = []
        ...
        for t_fr, wp_fr in targets_fr.items():
            ...
            oLp_path_fr = self._safe_oLp_path(oLp_set_seq, loop = False, allow_non_unique = True)
            ...
            if oLp_path_fr[-1] != oLp_path_center[0]:
                if len(oLp_path_fr) == 1:
                    ...
                    raise NotImplementedError("MM Doesn't support middle-injection beams")
            else:
                inv_start = False
        ...
        for t_to, wp_to in targets_to.items():
            ...
            oLp_path_to = self._safe_oLp_path(oLp_set_seq, loop = False, allow_non_unique = True)
