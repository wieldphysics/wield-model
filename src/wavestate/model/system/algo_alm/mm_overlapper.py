...
from transient.utilities.strings import table
from ... import optics
...
class ModeMatchingOverlapper(object):
    ...
    def overlap_table(self, first = (), last = ()):
        ...
        for t1 in targets:
            ...
            for t2 in targets:
                olaps2.append(
                    abs(self.overlap_field_between(t1, t2))**2
                )
            olaps.append(olaps2)
    ...
    def gouy_table(self, objects = None, **kwargs):
        ...
        for idx_to, descB_to in enumerate(descB_list):
            ...
            gouy_diff = []
            length_diff = []
            gouy_list.append(np.angle(descB_to.q_start.gouy_phasor / q_start.gouy_phasor, deg = True))
            ...
            for idx_fr, descB_fr in enumerate(descB_list):
                if idx_fr < idx_to:
                    gouy_diff.append(
                        np.angle(
                            (descB_to.q_start.gouy_phasor / descB_fr.q_start.gouy_phasor),
                            deg = True,
                        )
                    )
                    length_diff.append(
                        descB_to.z1_m - descB_fr.z2_m
                    )
