...
class PhysicsACAlgorithm(object):
    ...
    def __call__(
        ...
        #ic(solvect.kmatrix)
        datavec_p = solvect.kmatrix[(demod, '+AC')][(base.FrequencyKey({}), '+AC')][..., 0, 0]
        datavec_m = solvect.kmatrix[(demod, '-AC')][(base.FrequencyKey({}), '+AC')][..., 0, 0]
        #TODO, check realness
        ...
        for Wk, wnval in self.basis_wavenumbers(with_keys = True):
            for Fk, fval in self.basis_frequencies_DC(with_keys = True):
                for Qk, conj in (('+AC', False), ('-AC', True)):
                    if not conj:
                        yield (Wk, Fk, Qk), (wnval, fval + self._ac_algo.fs.AC_fspan, conj)
                    else:
                        yield (Wk, Fk, Qk), (wnval, fval - self._ac_algo.fs.AC_fspan, conj)
        return
