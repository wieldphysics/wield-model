...
class ComplexBeamParam(object):
    ...
    def cplg02(self):
        ...
        return (
            ...
            / self.ZR
            * (self.gouy_phasor / self.gouy_phasor.conjugate())
        )
    ...
    def overlap(self, q2):
        ...
        """
        return (
            abs(4 * self.value.imag * q2.value.imag)
            / abs(self.value.conjugate() - q2.value) ** 2
        )
    ...
    def overlap_HG(self, other):
        ...
        return (
            (
                (self.value * other.value.conjugate())
                / (other.value.conjugate() - self.value)
    ...
    def __str__(self):
        if self.R_inv != 0:
            ...
            R_str = u"1/0"
        return (
            u"Q(ZR={ZR}, Z={Z}, W0={W0}, W={W}, R={R}, λ={wavelength_m:.0f}nm)".format(
...
def beam_shape_1D(x, x_cbp, trans, tilt):
    ...
    return (
        ...
        * (1 / x_cbp.W) ** 0.5
        * np.exp(
            -1j * k * (x + trans) ** 2 / (2 * cbp)
            + 2 * np.pi * 1j * x / x_cbp.wavelength_m * np.sin(tilt)
        )
    )
