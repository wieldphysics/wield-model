...
class PhysicsAlgorithm(object):
    ...
    def _setup_optical(self):
        ...
        kg_frequency_DC_optical = base.KeyGroup(
            "frequency",
            sorted(self.fs.freq_set_optical, key=lambda fd: self.fs.frequency_map[fd]),
        )
        kg_wavenumber = base.KeyGroup(
            "wavenumber",
            sorted(
                self.fs.freq_set_wavenumbers, key=lambda wd: -self.fs.wavenumber_map[wd]
            ),
        )
    ...
    def _setup_mechanical(self):
        # HMM
        self.kg_frequency_DC_mechanical = base.KeyGroup(
            "frequency",
            sorted(
                self.fs.freq_set_mechanical, key=lambda fd: self.fs.frequency_map[fd]
            ),
        )
    ...
    def _setup_electrical(self):
        # HMM
        self.kg_frequency_DC_electrical = base.KeyGroup(
            "frequency",
            sorted(
                self.fs.freq_set_electrical, key=lambda fd: self.fs.frequency_map[fd]
            ),
        )
    ...
    def _setup_signal(self):
        # HMM
        self.kg_frequency_DC_signal = base.KeyGroup(
            "frequency",
            sorted(self.fs.freq_set_signal, key=lambda fd: self.fs.frequency_map[fd]),
        )
