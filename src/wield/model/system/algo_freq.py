    ...
    def _assemble_frequencies(self):
        ...
        for fname, F_obj in frequencies.items():
            ...
            if sckey not in sc_dict_optical:
                sc_dict_optical.setdefault(
                    sckey, self.pbg.resolve_parameter(F_obj, "order_optical")
                )
        ...
        for fname, F_obj in frequencies.items():
            ...
            if sckey not in sc_dict_signal:
                sc_dict_signal.setdefault(
                    sckey, self.pbg.resolve_parameter(F_obj, "order_signal")
                )
        ...
        for fname, F_obj in frequencies.items():
            ...
            if sckey not in sc_dict_electrical:
                sc_dict_electrical.setdefault(
                    sckey, self.pbg.resolve_parameter(F_obj, "order_electrical")
                )
        ...
        for fname, F_obj in frequencies.items():
            ...
            if sckey not in sc_dict_mechanical:
                sc_dict_mechanical.setdefault(
                    sckey, self.pbg.resolve_parameter(F_obj, "order_mechanical")
                )
        ...
        for fname, F_obj in wavelengths.items():
            ...
            if sckey not in sc_dict_wavenumbers:
                sc_dict_wavenumbers.setdefault(
                    sckey, self.pbg.resolve_parameter(F_obj, "order")
                )
    ...
    def _complete_frequencies(
        ...
        for N_combinations in range(1, 1 + len(fnames_all)):
            for fnames in itertools.combinations(fnames_all, N_combinations):
                ...
                if balanced:
                    for Fvect in itertools.product(
                        *[(freq_basis[fname], -freq_basis[fname]) for fname in fnames]
                    ):
                        Fsum = np.sum(Fvect)
                        add_one_each(Fsum)
                else:
