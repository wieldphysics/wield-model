...
class BondGraphAlgorithm(object):
    def __init__(self, pbg, log=_base.log_default):
        ...
        with self.log.heading("Bond Graph"):
            ...
            bond_edges_map, obj_ports = self._fully_forward_edges(
                edge_list, anti_edge_list=anti_edge_list
            )
    ...
    def rAp2oLp_set(self, refstr, dir=None, obj=None):
        ...
        if "!" in refstr:
            ...
            try:
                ...
                self.port_forward_mapping = self._fully_forward_ports(
                    [op1], pmap=self.port_forward_mapping
                )
            ...
            raise RuntimeError(
                ("Unrecognized port '{}', did you forget a trailing '/'?").format(
                    refstr
                )
            )
        ...
        while to_check:
            ...
            while True:
                ...
                if pret is None:
                    ...
                    raise RuntimeError(
                        "Port name {} of {} not acceptable".format(
                            pstr, self.pbg.path_str(obj)
                        )
                    )
    ...
    def _fully_forward_edges(self, edge_list, anti_edge_list=[]):
        ...
        for (op1, op2), antiVal in anti_bond_edges_map.items():
            if antiVal > 0:
                raise RuntimeError(
                    "More anti-bonds than bonds for edge ({}, {}).format(op1, op2)"
                )
    ...
    def _parse_bond_list(self, obj, bl):
        ...
        if "-" in first:
            raise RuntimeError(
                ("First port in a sequence cannot be a chained port {}").format(first)
            )
        if "-" in last:
            raise RuntimeError(
                ("Last port in a sequence cannot be a chained port {}").format(last)
            )
        ...
        for mid_chain in true_lst[idx_firstmid:-1]:
            if "-" not in mid_chain:
                raise RuntimeError(
                    ("port in the middle of sequence must be a chained port {}").format(
                        mid_chain
                    )
                )
...
class PortInformationAlgorithmManipulator(BondGraphAlgorithmView):
    ...
    def gen_optical_port(self, pname, lprefix):
        self.port_inform(
            pname,
            base.port_types.BGT_OPTICS_PHYSICAL,
    ...
    def gen_signal_out_port(self, pname, lprefix):
        self.port_inform(
            pname,
            base.port_types.BGT_SIGNAL_OUT,
